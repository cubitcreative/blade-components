...
<div>
    <label class="cursor-pointer flex items-center group select-none @isset($spread) justify-between @endisset">
        ...
        <div class="@if($pulse ?? false) animate-pulse @endif">
            @if(trim($slot))
                <div class="mr-4">
                    <{{ $header ?? 'h4' }} class="text-{{ $color ?? 'brand1' }}-600">{{ $slot }}</{{ $header ?? 'h4' }}>
                    @isset($subtitle)
                        <p class="text-sm text-grey-light leading-none" style="margin-top:-2px;">
                            {{ $subtitle }}
                        </p>
                    @endisset
                </div>
        ...
        <input type="checkbox" 
            class="{{ $size ?? 'w-16 h-10' }} cursor-pointer rounded border-gray-300 shadow-sm focus:ring focus:ring-offset-0 focus:ring-opacity-50 text-2xl {{ isset($color) ? $colors[$color] : $color['brand1'] }} @if($pulse ?? false) animate-pulse @endif" 
            {{ $attributes }}
        >
